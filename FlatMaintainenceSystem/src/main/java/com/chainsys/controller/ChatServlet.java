...
public class ChatServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);
             PreparedStatement ps = conn.prepareStatement("SELECT Messages.id, Users.username, Messages.message FROM Messages INNER JOIN Users ON Messages.user_id = Users.id WHERE Messages.id > ? ORDER BY Messages.id")) {
        ...
        for (Message msg : messages) {
            plainTextResponse.append(msg.username).append(": ").append(msg.message).append("\n");
        }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (username != null && !username.trim().isEmpty() && message != null && !message.trim().isEmpty()) {
            try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD)) {
                ...
                if (userId == -1) {
                    try (PreparedStatement ps = conn.prepareStatement("INSERT INTO Users (username) VALUES (?)", Statement.RETURN_GENERATED_KEYS)) {
                ...
                try (PreparedStatement ps = conn.prepareStatement("INSERT INTO Messages (user_id, message) VALUES (?, ?)")) {
                    ps.setInt(1, userId);
